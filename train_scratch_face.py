...
def train(model, task, trait, timestamp, output_path):
    ...
    for fold in range(fold_num):
        ...
        for epoch in range(EPOCHS):
            ...
            net.train()
            for _, s_face_batch, _, i_face_batch, y_batch in train_loader:
                if torch.cuda.is_available():
                ...
                y_out = net(s_face_batch, i_face_batch)
            ...
            for _, s_face_batch, _, i_face_batch, y_batch in test_loader:
                if torch.cuda.is_available():
                    s_face_batch = s_face_batch.permute(0, 1, 4, 2, 3)
                    s_face_batch = s_face_batch.reshape(-1, 3, 224, 224)
                    i_face_batch = i_face_batch.permute(0, 1, 4, 2, 3)
...
if __name__ == '__main__':
    ...
    # configure training
    models = {
        'MyModelFace': nets_scratch.MyModelFace,
    }
